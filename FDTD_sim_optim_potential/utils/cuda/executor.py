...
class executor ():
    ...
    def config_geometries(self, layer_thickness, maxDistEffect, airAbsorptivity = 0):
        try:
            ...
            self.maxAbsorptivity = np.array([5 / self.dt]).astype(self.VarType)[0] #[0.5 / self.dt]).astype(self.VarType)[0]
    ...
    def config_simulation(self, dt, ds, nPoints, density, c, grid_limits, times, ratio_times):
        try:
            ...
            self.time = np.array([0.0]).astype(self.VarType)[0]
    ...
    def init_grid(self):
        try:
            ...
            self.manager.potential = cuda.to_device(np.zeros((self.nPoints * self.nPoints * self.nPoints), dtype = self.var_type) + 1j*np.zeros((self.nPoints * self.nPoints * self.nPoints), dtype = self.var_type), stream = self.stream)
            self.manager.save_potential = cuda.to_device(np.zeros((self.nPoints * self.nPoints * self.nPoints), dtype = self.var_type) + 1j*np.zeros((self.nPoints * self.nPoints * self.nPoints), dtype = self.var_type), stream = self.stream)
    ...
    def simulation_step (self):
        try:
            ...
            #self.stream.synchronize()
            #
            #self.calculator.step_velocity_values(self.manager.velocity, self.manager.velocity_b, self.manager.pressure,
            ...
            #self.calculator.step_velocity_values_n_emitters(self.manager.velocity, self.manager.pressure, self.manager.geometry_field, 
            #                                                self.dt, self.ds, self.density, 
            ...
            #print('a')
            self.calculator.step_all(self.manager.potential, self.manager.save_potential, self.manager.geometry_field, 
                                                            self.dt, self.ds, self.c**2, 
                                                            self.manager.emitters_normal, self.manager.emitters_amplitude, self.manager.emitters_frequency,
                                                            self.manager.emitters_phase, self.time)
            #print('b')
            #self.stream.synchronize()
        ...
        try:
            ...
            while self.time - self.key_times['thermalization'] <= self.key_times['simulation']:
                ...
                if self.time - self.key_times['thermalization'] >= self.key_times['record_times'][0] and self.time - self.key_times['thermalization'] <= self.key_times['record_times'][1]:
                    ...
                    self.plotter.record(self.manager.potential, self.manager.save_potential, self.density/self.dt, self.time - self.key_times['thermalization'] - self.key_times['record_times'][0], self.ratio_times)
                else:
