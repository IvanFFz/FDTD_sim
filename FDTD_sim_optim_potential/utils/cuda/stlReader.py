...
from numba import cuda, int64 as nbint64 #, float32, float64, void
...
def results_to_list_noreturn_cuda (results_grid, geometry_points, grid_min, mesh_min, count):
	...
	#idx = cuda.local.array((MAXTHREADNUMBER,MAXTHREADNUMBER,MAXTHREADNUMBER), nbint64)
		#shared_position[0] = 0
	...
	#	if results_grid[i, j, k] == 1:
			...
			#cuda.atomic.exch(idx, (cuda.threadIdx.x, cuda.threadIdx.y, cuda.threadIdx.z), cuda.atomic.add(count, 0, 1))
	#		idx[cuda.threadIdx.x, cuda.threadIdx.y, cuda.threadIdx.z] = cuda.atomic.add(count, 0, 1)
			
	#cuda.syncthreads()
	if i < results_grid.shape[0] and j < results_grid.shape[1] and k < results_grid.shape[2] and count[0] < geometry_points.shape[0] / 3:
		if results_grid[i, j, k] == 1:
