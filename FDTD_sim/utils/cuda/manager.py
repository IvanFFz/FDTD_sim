...
def PML_limit_volume_no_return_cuda (absorption, maxDist, maxValue, minValue):
	...
	if i<absorption.shape[0] and j<absorption.shape[1] and k<absorption.shape[2]:
		...
		if (i<maxDist or i>=absorption.shape[0]-maxDist) or (j<maxDist or j>=absorption.shape[1]-maxDist) or (k<maxDist or k>=absorption.shape[2]-maxDist):
			cuda.atomic.max(absorption,
							(i,j,k),
							(maxValue - minValue)*(1 - min(i, j, k, absorption.shape[0]-i-1, absorption.shape[1]-j-1, absorption.shape[2]-k-1)/maxDist) + minValue)
		
#Then use concatenate arrays
#Emitters properties defined as -> Amplitude, Frequency, Phase <- In that order			
def set_point_as_emitter_no_return_cuda (emitters_amplitude, emitters_frequency, emitters_phase, emitters_normal_x, emitters_normal_y, emitters_normal_z, geometry_points, amplitude, frequency, phase, emitters_normal):
	...
	if i<geometry_points.shape[0]:
		...
		emitters_frequency[geometry_points[i, 0], geometry_points[i, 1], geometry_points[i, 2]] = 2 * np.pi * frequency
		emitters_phase[geometry_points[i, 0], geometry_points[i, 1], geometry_points[i, 2]] = 2 * np.pi *phase / 360
		emitters_normal_x[geometry_points[i, 0], geometry_points[i, 1], geometry_points[i, 2]] = emitters_normal[0]
		emitters_normal_y[geometry_points[i, 0], geometry_points[i, 1], geometry_points[i, 2]] = emitters_normal[1]
		emitters_normal_z[geometry_points[i, 0], geometry_points[i, 1], geometry_points[i, 2]] = emitters_normal[2]
	...
	def config_manager_functions (self):
		try:
			self.config = {
				...
				'set_point_as_emitter':								cuda.jit('void('+self.VarType+'[:,:,:], '+self.VarType+'[:,:,:], '+self.VarType+'[:,:,:], ' 
																					+self.VarType+'[:,:,:], '+self.VarType+'[:,:,:], '+self.VarType+'[:,:,:], int64[:,:], '
	...
	def set_point_as_emitter(self, geometry_points, amplitude, frequency, phase, normal_emission):
		...
		try:
			...
			self.config['set_point_as_emitter'][self.griddim, self.blockdim, self.stream](
				self.emitters_amplitude, self.emitters_frequency, self.emitters_phase, self.emitters_normal_x, self.emitters_normal_y, self.emitters_normal_z, geometry_points, amplitude, frequency, phase, normal_emission)
